...
This example is an int8 version of the dense matrix multiply (`Z = A * B`) for the Electron E1x general-purpose processor. It computes the same operation as the base `dmm` example but on 8-bit integer inputs, with a quantization step applied to the results.
...
| `multiplier`     | `1`     | This is the integer multiplier applied during requantization of each result. Changing it rescales the 8-bit output.                                                                      |
| `shift`          | `0`     | This is the right-shift amount applied during requantization. It sets the fixed-point scale together with `multiplier`.                                                                  |
| `MAT_REF_SIZE`   | `32`    | This is the matrix dimension (used for all of `n`, `m`, and `o`), from the shared `mat.h`. It sets the problem size and must stay divisible by 4 for the packing and tiling to be valid. |

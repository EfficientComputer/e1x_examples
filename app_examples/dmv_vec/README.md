...
This example is a vectorized version of the dense matrix-vector multiply (`z = A · b`) for the Electron E1x general-purpose processor. It computes the same result as the base `dmv` example but packs the arithmetic into SIMD dot products.
...
| `MAT_REF_SIZE`   | `32`    | This is the matrix dimension (both rows and columns), from the shared `mat.h`. It sets the problem size and must stay divisible by 4 for the row unrolling to be valid. |

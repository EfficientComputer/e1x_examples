...
A vectorized version of the 5x5 2D convolution for the Electron E1x general-purpose processor. It computes the same result as the base `conv5x5` example, sliding a 5x5 filter across an input image, but packs the per-pixel arithmetic into SIMD dot products.
...
| `CONV5x5_N_PAD`            | `N + 4` | The padded side length of the input buffer. The filter is 5 wide, so the buffer is padded by 4. Keep it as `N + 4`.    |
...
| `CONV5x5_RANGE`            | `8`     | The range of the pseudo-random filter weights, centered around zero. Keeping it small helps the values fit in 16 bits. |

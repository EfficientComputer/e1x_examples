...
A vectorized version of the depthwise 3x3 convolution for the Electron E1x general-purpose processor. It computes the same result as the base `conv3x3_dw` example, applying one independent 3x3 filter per channel, but packs the arithmetic into SIMD dot products.

...
This example adds two **sparse matrices** (`Z = A + B`) on the Electron E1 general-purpose processor, producing a dense output matrix. Both inputs are stored in compressed sparse row (CSR) format, so only the nonzero entries are processed. It shows how irregular, data-dependent access maps onto the Fabric architecture.

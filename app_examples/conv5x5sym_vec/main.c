...
static void dconv5x5sym(const data_t *inp, const data_t *filter, data_t *out,
                        int n, int tile_rows)
{
    // see conv3x3
    ...
    for (int row_start = 0; row_start < n; row_start += tile_rows)
    {
        ...
        dconv5x5sym_inner((data_t *)inp + row_start * n,
                          out + (row_start - 5) * n, n, tile_rows + 4, f00, f10,
                          f20, f01, f11, f21, f02, f12, f22);
    }
}

void dconv_ref(const data_t *inp, int n, const data_t *filter, int d,
               data_t *out)
{
    for (int i = 0; i <= n - d; i++)
    {
        for (int j = 0; j <= n - d; j++)
        {
            ...
            for (int fi = 0; fi < d; fi++)
            {
                for (int fj = 0; fj < d; fj++)
                {
                    ...
                    sum += filter[fi + fj * d] * inp[col + row * n];
                }
            }
...
int main()
{
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; iter++)
        dconv5x5sym(in, filter_global, test_out + 5 * N, N, TILE_ROWS);
    ...
    for (int i = 0; i < N - 4; i++)
    {
        for (int j = 0; j < N - 4; j++)
        {
            if ((int)ref_out[i + j * N] != (int)(test_out + 5 * N)[i + j * N])
            {
                printf("[conv5x5] FAIL\n");
                return 1;
            }

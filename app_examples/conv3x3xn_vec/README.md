...
A vectorized version of the multi-channel 3x3 convolution for the Electron E1 general-purpose processor. It computes the same result as the base `conv3x3xn` example, convolving a 3x3 filter over every input channel and summing across channels, but packs the arithmetic into SIMD dot products.

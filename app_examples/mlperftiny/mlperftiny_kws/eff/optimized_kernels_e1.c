...
extern uint32_t load_guard_i32(bool valid, uint32_t a, uint32_t b);
...
// 2x2x5 vec4 is pretty low utilization of 36 AR+AP units
// increasing OCR to 3 is probably the best option here?
// NOTE: Assumes zero-point has been folded into bias.
// could also try 7x1x5 if just using i16 loads isn't too bad.
__effcc_rip_exact void conv_kws_layer_0(const int8_t *restrict in,
    ...
    // Tile sizes
    const uint32_t FHR = 2;
    const uint32_t OCR = 2;
    ...
    /*
        ...
        + append loop iterator name to pointer name
            - e.g. out_oc is evuivalent to out + oc * <oc stride>
    ...
    __effcc_ignore_memory_order {
        ...
        for (int oc = 0; oc < OC; oc += OCR) {
            ...
            for (int oh = 0; oh < OH; oh++) {
                ...
                int32_t acc1_ow0 = bias_oc[1];
                int32_t acc1_ow1 = acc1_ow0;
                int32_t acc1_ow2 = acc1_ow0;
                int32_t acc1_ow3 = acc1_ow0;
                int32_t acc1_ow4 = acc1_ow0;
                ...
                for (int fh = 0; fh < FH; fh += FHR) {
                    /*
                        ...
                        */
                    word4b_t in_fh0_210z, in_fh0_4321, in_fh0_6543, in_fh0_8765,
                        in_fh0_z987, in_fh1_210z, in_fh1_4321, in_fh1_6543,
                        in_fh1_8765, in_fh1_z987, f_oc0_fh0, f_oc0_fh1,
                        f_oc1_fh0, f_oc1_fh1;
                    ...
                    f_oc1_fh0.u32 =
                        *((uint32_t *)(filter_oc_fh + 1 * FH * FW + 0 * FW));
                    f_oc1_fh1.u32 =
                        *((uint32_t *)(filter_oc_fh + 1 * FH * FW + 1 * FW));
                    ...
                    // 0-9A-J (20 elements across two 10 element rows)
                    // z0123456789z : row 0
                    // zABCDEFGHIJz : row 1
                    // variables list bytes-in-word in reverse order (e.g. 3210)
                    // (most significant byte to least significant byte)
                    // to make shifting and masking logic more intuitive.
                    word4b_t in_3210, in_7654, in_BA98, in_FEDC, in_JIHG;
                    word4b_t zp_v;
                    zp_v.vi8[0] = IZP;
                    zp_v.vi8[1] = IZP;
                    zp_v.vi8[2] = IZP;
                    ...
                    // Load 4 elements at a time via i32 to reduce bank
                    // conflicts when accessing different i8s from the same
                    // word.
                    in_3210.u32 = load_guard_i32(
                        row0_valid, *((uint32_t *)(in_oh_fh + 0 * VEC)),
                        zp_v.u32);
                    in_7654.u32 = load_guard_i32(
                        row0_valid, *((uint32_t *)(in_oh_fh + 1 * VEC)),
                        zp_v.u32);
                    in_BA98.u32 = load_guard_i32(
                        row0_valid || row1_valid,
                        *((uint32_t *)(in_oh_fh + 2 * VEC)), zp_v.u32);
                    in_FEDC.u32 = load_guard_i32(
                        row1_valid, *((uint32_t *)(in_oh_fh + 3 * VEC)),
                        zp_v.u32);
                    in_JIHG.u32 = load_guard_i32(
                        row1_valid, *((uint32_t *)(in_oh_fh + 4 * VEC)),
                        zp_v.u32);

                    // reassign upper/lower parts of BA98 depending on row
                    // validty
                    in_BA98.vi16[0] =
                        row0_valid ? in_BA98.vi16[0] : zp_v.vi16[0];
                    in_BA98.vi16[1] =
                        row1_valid ? in_BA98.vi16[1] : zp_v.vi16[1];
                    ...
                    // Bytes are listed in from most to least significant.
                    in_fh0_210z.u32 = (in_3210.u32 << 1 * 8) | ((uint32_t)IZP);
                    in_fh0_4321.u32 =
                        (in_7654.u32 << 3 * 8) | (in_3210.u32 >> 1 * 8);
                    in_fh0_6543.u32 =
                        (in_7654.u32 << 1 * 8) | (in_3210.u32 >> 3 * 8);
                    in_fh0_8765.u32 =
                        (in_BA98.u32 << 3 * 8) | (in_7654.u32 >> 1 * 8);
                    in_fh0_z987.u32 = ((uint32_t)IZP << 3 * 8) |
                                      ((in_BA98.u32 & 0x0000FFFFU) << 1 * 8) |
                                      (in_7654.u32 >> 3 * 8);

                    // Row 1 starts at in_BA98 byte 2 (2-byte offset into loaded
                    // words). Same window pattern as fh0 but using FEDC/JIHG as
                    // the source pair, with shift amounts offset by 2 bytes
                    // accordingly.
                    //  KJIH IHGF GFED EDCB CBAz
                    in_fh1_210z.u32 = (in_FEDC.u32 << 3 * 8) |
                                      ((in_BA98.u32 & 0xFFFF0000U) >> 1 * 8) |
                                      ((uint32_t)IZP);  // CBA9
                    in_fh1_4321.u32 =
                        (in_FEDC.u32 << 1 * 8) | (in_BA98.u32 >> 3 * 8);
                    in_fh1_6543.u32 =
                        (in_JIHG.u32 << 3 * 8) | (in_FEDC.u32 >> 1 * 8);
                    in_fh1_8765.u32 =
                        (in_JIHG.u32 << 1 * 8) | (in_FEDC.u32 >> 3 * 8);
                    in_fh1_z987.u32 =
                        ((uint32_t)IZP << 3 * 8) | (in_JIHG.u32 >> 1 * 8);

                    acc_ow0 += sdot_vec4xi8(in_fh0_210z.vi8, f_oc0_fh0.vi8);
                    acc_ow1 += sdot_vec4xi8(in_fh0_4321.vi8, f_oc0_fh0.vi8);
                    acc_ow2 += sdot_vec4xi8(in_fh0_6543.vi8, f_oc0_fh0.vi8);
                    acc_ow3 += sdot_vec4xi8(in_fh0_8765.vi8, f_oc0_fh0.vi8);
                    acc_ow4 += sdot_vec4xi8(in_fh0_z987.vi8, f_oc0_fh0.vi8);

                    acc_ow0 += sdot_vec4xi8(in_fh1_210z.vi8, f_oc0_fh1.vi8);
                    acc_ow1 += sdot_vec4xi8(in_fh1_4321.vi8, f_oc0_fh1.vi8);
                    acc_ow2 += sdot_vec4xi8(in_fh1_6543.vi8, f_oc0_fh1.vi8);
                    acc_ow3 += sdot_vec4xi8(in_fh1_8765.vi8, f_oc0_fh1.vi8);
                    acc_ow4 += sdot_vec4xi8(in_fh1_z987.vi8, f_oc0_fh1.vi8);

                    acc1_ow0 += sdot_vec4xi8(in_fh0_210z.vi8, f_oc1_fh0.vi8);
                    acc1_ow1 += sdot_vec4xi8(in_fh0_4321.vi8, f_oc1_fh0.vi8);
                    acc1_ow2 += sdot_vec4xi8(in_fh0_6543.vi8, f_oc1_fh0.vi8);
                    acc1_ow3 += sdot_vec4xi8(in_fh0_8765.vi8, f_oc1_fh0.vi8);
                    acc1_ow4 += sdot_vec4xi8(in_fh0_z987.vi8, f_oc1_fh0.vi8);

                    acc1_ow0 += sdot_vec4xi8(in_fh1_210z.vi8, f_oc1_fh1.vi8);
                    acc1_ow1 += sdot_vec4xi8(in_fh1_4321.vi8, f_oc1_fh1.vi8);
                    acc1_ow2 += sdot_vec4xi8(in_fh1_6543.vi8, f_oc1_fh1.vi8);
                    acc1_ow3 += sdot_vec4xi8(in_fh1_8765.vi8, f_oc1_fh1.vi8);
                    acc1_ow4 += sdot_vec4xi8(in_fh1_z987.vi8, f_oc1_fh1.vi8);
                ...
                out_oc_oh[0 * OC + 1] = acc1_ow0;
                out_oc_oh[1 * OC + 1] = acc1_ow1;
                out_oc_oh[2 * OC + 1] = acc1_ow2;
                out_oc_oh[3 * OC + 1] = acc1_ow3;
                out_oc_oh[4 * OC + 1] = acc1_ow4;
...
__effcc_rip_exact void depthwise_conv_kws_layer_1(
    ...
    const uint32_t OHR = 1;

    const uint32_t ICV = 4;
    const uint32_t OCV = 4;
...
__effcc_rip_exact void conv2d_1x1(
    ...
    // unroll OHWR, OCR
    const uint32_t ICV = 4;
    const uint32_t OHWR = 4;
    const uint32_t OCR = 8;
        ...
        for (int oc = 0; oc < (OC / OCR); oc++) {
            ...
            for (int ohw = 0; ohw < ((OHW + OHWR - 1) / OHWR); ohw++) {
                ...
                for (int ic = 0; ic < (IC / ICV); ic++) {
                    vec4xi8_t in_p0_v, in_p1_v, in_p2_v, in_p3_v;
                    vec4xi8_t filt_c0_v, filt_c1_v, filt_c2_v, filt_c3_v;
                    vec4xi8_t filt_c4_v, filt_c5_v, filt_c6_v, filt_c7_v;

                    in_p0_v = *((vec4xi8_t *)(in_ohw_ic + 0 * IC));
                    // know that 125 % 4 is 1 < 4, so check last tile for
                    // p1/p2/p3
                    in_p1_v = __builtin_expect(ohw < (OHW / OHWR), 1)
                                  ? *((vec4xi8_t *)(in_ohw_ic + 1 * IC))
                                  : 0;
                    in_p2_v = __builtin_expect(ohw < (OHW / OHWR), 1)
                                  ? *((vec4xi8_t *)(in_ohw_ic + 2 * IC))
                                  : 0;
                    in_p3_v = __builtin_expect(ohw < (OHW / OHWR), 1)
                                  ? *((vec4xi8_t *)(in_ohw_ic + 3 * IC))
                                  : 0;

                    filt_c0_v = *((vec4xi8_t *)(filter_oc_ic + 0 * IC));
                    filt_c1_v = *((vec4xi8_t *)(filter_oc_ic + 1 * IC));
                    filt_c2_v = *((vec4xi8_t *)(filter_oc_ic + 2 * IC));
                    filt_c3_v = *((vec4xi8_t *)(filter_oc_ic + 3 * IC));
                    filt_c4_v = *((vec4xi8_t *)(filter_oc_ic + 4 * IC));
                    filt_c5_v = *((vec4xi8_t *)(filter_oc_ic + 5 * IC));
                    filt_c6_v = *((vec4xi8_t *)(filter_oc_ic + 6 * IC));
                    filt_c7_v = *((vec4xi8_t *)(filter_oc_ic + 7 * IC));

                    acc_p0_c0 += sdot_vec4xi8(in_p0_v, filt_c0_v);
                    acc_p0_c1 += sdot_vec4xi8(in_p0_v, filt_c1_v);
                    acc_p0_c2 += sdot_vec4xi8(in_p0_v, filt_c2_v);
                    acc_p0_c3 += sdot_vec4xi8(in_p0_v, filt_c3_v);
                    acc_p0_c4 += sdot_vec4xi8(in_p0_v, filt_c4_v);
                    acc_p0_c5 += sdot_vec4xi8(in_p0_v, filt_c5_v);
                    acc_p0_c6 += sdot_vec4xi8(in_p0_v, filt_c6_v);
                    acc_p0_c7 += sdot_vec4xi8(in_p0_v, filt_c7_v);

                    acc_p1_c0 += sdot_vec4xi8(in_p1_v, filt_c0_v);
                    acc_p1_c1 += sdot_vec4xi8(in_p1_v, filt_c1_v);
                    acc_p1_c2 += sdot_vec4xi8(in_p1_v, filt_c2_v);
                    acc_p1_c3 += sdot_vec4xi8(in_p1_v, filt_c3_v);
                    acc_p1_c4 += sdot_vec4xi8(in_p1_v, filt_c4_v);
                    acc_p1_c5 += sdot_vec4xi8(in_p1_v, filt_c5_v);
                    acc_p1_c6 += sdot_vec4xi8(in_p1_v, filt_c6_v);
                    acc_p1_c7 += sdot_vec4xi8(in_p1_v, filt_c7_v);

                    acc_p2_c0 += sdot_vec4xi8(in_p2_v, filt_c0_v);
                    acc_p2_c1 += sdot_vec4xi8(in_p2_v, filt_c1_v);
                    acc_p2_c2 += sdot_vec4xi8(in_p2_v, filt_c2_v);
                    acc_p2_c3 += sdot_vec4xi8(in_p2_v, filt_c3_v);
                    acc_p2_c4 += sdot_vec4xi8(in_p2_v, filt_c4_v);
                    acc_p2_c5 += sdot_vec4xi8(in_p2_v, filt_c5_v);
                    acc_p2_c6 += sdot_vec4xi8(in_p2_v, filt_c6_v);
                    acc_p2_c7 += sdot_vec4xi8(in_p2_v, filt_c7_v);

                    acc_p3_c0 += sdot_vec4xi8(in_p3_v, filt_c0_v);
                    acc_p3_c1 += sdot_vec4xi8(in_p3_v, filt_c1_v);
                    acc_p3_c2 += sdot_vec4xi8(in_p3_v, filt_c2_v);
                    acc_p3_c3 += sdot_vec4xi8(in_p3_v, filt_c3_v);
                    acc_p3_c4 += sdot_vec4xi8(in_p3_v, filt_c4_v);
                    acc_p3_c5 += sdot_vec4xi8(in_p3_v, filt_c5_v);
                    acc_p3_c6 += sdot_vec4xi8(in_p3_v, filt_c6_v);
                    acc_p3_c7 += sdot_vec4xi8(in_p3_v, filt_c7_v);
                ...
                out_oc_ohw[0 * OC + 4] = acc_p0_c4;
                out_oc_ohw[0 * OC + 5] = acc_p0_c5;
                out_oc_ohw[0 * OC + 6] = acc_p0_c6;
                out_oc_ohw[0 * OC + 7] = acc_p0_c7;
                ...
                if (__builtin_expect(ohw < (OHW / OHWR), 1)) {
                    ...
                    out_oc_ohw[1 * OC + 4] = acc_p1_c4;
                    out_oc_ohw[1 * OC + 5] = acc_p1_c5;
                    out_oc_ohw[1 * OC + 6] = acc_p1_c6;
                    out_oc_ohw[1 * OC + 7] = acc_p1_c7;

                    out_oc_ohw[2 * OC + 0] = acc_p2_c0;
                    out_oc_ohw[2 * OC + 1] = acc_p2_c1;
                    out_oc_ohw[2 * OC + 2] = acc_p2_c2;
                    out_oc_ohw[2 * OC + 3] = acc_p2_c3;
                    out_oc_ohw[2 * OC + 4] = acc_p2_c4;
                    out_oc_ohw[2 * OC + 5] = acc_p2_c5;
                    out_oc_ohw[2 * OC + 6] = acc_p2_c6;
                    out_oc_ohw[2 * OC + 7] = acc_p2_c7;

                    out_oc_ohw[3 * OC + 0] = acc_p3_c0;
                    out_oc_ohw[3 * OC + 1] = acc_p3_c1;
                    out_oc_ohw[3 * OC + 2] = acc_p3_c2;
                    out_oc_ohw[3 * OC + 3] = acc_p3_c3;
                    out_oc_ohw[3 * OC + 4] = acc_p3_c4;
                    out_oc_ohw[3 * OC + 5] = acc_p3_c5;
                    out_oc_ohw[3 * OC + 6] = acc_p3_c6;
                    out_oc_ohw[3 * OC + 7] = acc_p3_c7;
                }
                out_oc_ohw += OHWR * OC;
                in_ohw += OHWR * IC;
            }  // end ohw loop

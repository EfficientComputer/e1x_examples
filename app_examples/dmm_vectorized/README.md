...
This example is a vectorized version of the dense matrix multiply (`Z = A * B`) for the Electron E1x general-purpose processor. It computes the same result as the base `dmm` example but packs the arithmetic into 16-bit SIMD dot products.
...
| `MAT_REF_SIZE`   | `32`    | This is the matrix dimension (used for all of `n`, `m`, and `o`), from the shared `mat.h`. It sets the problem size and must stay divisible by 4 for the tiling to be valid. |

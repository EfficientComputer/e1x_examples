...
const int32_t EXPECTED_CHECKSUM = 146072;
...
int main()
{
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; iter++)
        dconv3x3(in, M, N, INSTRIDE, filter_global, test_out);

    int32_t checksum = 0;
    for (int i = 0; i < M - 2; i++)
        for (int j = 0; j < N - 2; j++)
            checksum += (int32_t)test_out[j + (i + 3) * OUTSTRIDE];

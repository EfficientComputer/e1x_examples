...
A real-time **model-predictive controller (MPC)** for a Crazyflie-class
quadrotor, running on E1x. This is [TinyMPC](https://tinympc.org) — ADMM-based
MPC with a precomputed infinite-horizon cache — reimplemented in bare-metal C and
fixed-point for the fabric.
...
- All arithmetic is **int32 Q20** fixed point (`int32 × int32 → int64` widening
  accumulate, then `>> 20`) — **no floating point, no 64-bit divide**.
...
It demonstrates a **complete, constrained, real-time optimal controller** — not a
microbenchmark — running in fixed point on the fabric. The same kernel sustains

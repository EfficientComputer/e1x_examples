...
This example performs a **breadth-first search** over a graph on the Electron E1 general-purpose processor. It labels every vertex with its discovery level (the number of hops from a chosen start vertex), showing how graph traversal maps onto the Fabric architecture.

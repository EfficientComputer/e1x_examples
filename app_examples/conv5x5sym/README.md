...
This example is a variant of the 5x5 2D convolution for the Electron E1x general-purpose processor. It computes the same operation as the base `conv5x5` example, sliding a 5x5 filter across an input image, but exploits filter symmetry to cut the number of multiplies per output pixel.

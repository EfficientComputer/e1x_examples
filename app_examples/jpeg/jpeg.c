...
#define FRAC_BITS 8
#define MUL_TYPE int32_t
#endif

// assumes x is quantized such that real_x = x / (1 << FRAC_BITS)
#define FIXED_POINT_MUL_CONST(x, real_y)                                   \
    ((fixed_point)(((MUL_TYPE)(x) *                                        \
                    (MUL_TYPE)((fixed_point)((real_y) *                    \
                                             (float)(1 << FRAC_BITS)))) >> \
                   FRAC_BITS))
#define FIXED_POINT_MUL(x, y) \
    ((fixed_point)(((MUL_TYPE)(x) * (MUL_TYPE)(y)) >> FRAC_BITS))

// -3 <= x <= 3
#define FIXED_POINT_DIV_CONST_SMALL_DIVIDEND(real_x, y) \
    ((fixed_point)(((fixed_point)(real_x) << (FRAC_BITS * 2 - 3)) / (y)) << 3)
#define FIXED_POINT_LITERAL(real_x) \
    ((fixed_point)((real_x) * (float)(1 << FRAC_BITS)))

#define FIXED_POINT_ROUND_TO_INT(x) \
    ((int)(((x) + (1 << (FRAC_BITS - 1))) >> FRAC_BITS))
#define FIXED_POINT_CAST_FROM_INT(x) \
    ((fixed_point)(((fixed_point)(x)) << FRAC_BITS))
                          ...
                          const unsigned short HTAC[256][2], int *pos)
{
    ...
    for (int i = 0; numDC > i; i++)
    {
        ...
        posLocal = stbiw__jpg_writeBits_direct(s, &bitBufLocal, &bitCntLocal,
                                               buf, posLocal);
    }
        ...
        if (du_val != 0)
        {
            ...
            for (int nrmarker = 0; nrmarker < lng + 2; ++nrmarker)
            {
                ...
                posLocal = stbiw__jpg_writeBits_direct(
                    s, &bitBufLocal, &bitCntLocal, buf, posLocal);
            }
            ...
            for (int xx = 0; xx < 8; ++xx, ++pos)
            {
                ...
                subUs[j][pos] =
                    FIXED_POINT_MUL_CONST((Us[j][k + 0] + Us[j][k + 1] +
                                          ...
                                          0.25f);
                subVs[j][pos] =
                    FIXED_POINT_MUL_CONST((Vs[j][k + 0] + Vs[j][k + 1] +
                                           Vs[j][k + 16] + Vs[j][k + 17]),
                                          0.25f);
            }
        }
    ...
    for (int j = 0; j < subIterCount; j++)
    {
            ...
            for (int row = y; row < y + 16; row += 1)
        {
            ...
            for (int col = x; col < x + 16; ++col)
            {
                ...
                fixed_point r = FIXED_POINT_CAST_FROM_INT(dataR[p]),
                            g = FIXED_POINT_CAST_FROM_INT(dataG[p]),
                            b = FIXED_POINT_CAST_FROM_INT(dataB[p]);
                Ys[j][pos] = FIXED_POINT_MUL_CONST(r, 0.29900f) +
                             FIXED_POINT_MUL_CONST(g, 0.58700f) +
                             FIXED_POINT_MUL_CONST(b, 0.11400f) -
                             FIXED_POINT_LITERAL(128);
                Us[j][pos] = FIXED_POINT_MUL_CONST(r, -0.16874f) -
                             FIXED_POINT_MUL_CONST(g, 0.33126f) +
                             FIXED_POINT_MUL_CONST(b, 0.50000f);
                Vs[j][pos] = FIXED_POINT_MUL_CONST(r, 0.50000f) -
                             FIXED_POINT_MUL_CONST(g, 0.41869f) -
                             FIXED_POINT_MUL_CONST(b, 0.08131f);
            }
        }
    }
}
...
static const int32_t aasf_quant[] = {
    262143, 363604, 342507, 308248, 262143, 205965, 141871, 72325,
    363604, 504333, 475071, 427553, 363604, 285681, 196781, 100318,
    342507, 475071, 447507, 402746, 342507, 269106, 185363, 94497,
...
#define CONST_SHIFT >> (QUANT_FRAC - FRAC_BITS)
            ...
            fixed_point yVal =
                FIXED_POINT_CAST_FROM_INT(YTable[stbiw__jpg_ZigZag[k]]) + zero;
            fixed_point uvVal =
                FIXED_POINT_CAST_FROM_INT(UVTable[stbiw__jpg_ZigZag[k]]) + zero;

            fdtbl_Y[k] = FIXED_POINT_DIV_CONST_SMALL_DIVIDEND(
                1, FIXED_POINT_MUL(
                       yVal, (int32_t)aasf_quant[row * 8 + col] CONST_SHIFT));
            fdtbl_UV[k] = FIXED_POINT_DIV_CONST_SMALL_DIVIDEND(
                1, FIXED_POINT_MUL(
                       uvVal, (int32_t)aasf_quant[row * 8 + col] CONST_SHIFT));
        }
    }
}
    ...
    int lastNonZeroIdxes[][6], int32_t DUs[][6][64])
{
    for (int j = 0; j < subIterCount * 6; j++)
    {
        ...
        int lastNonZero =
            stbiw__jpg_dct_DU(DUs[div6][mod6], CDU, duStride, fdtbl);
    ...
    int lastNonZeroIdxes[][3], int32_t DUs[][3][64])
{
    for (int j = 0; j < subIterCount * 3; j++)
    {
        ...
        int lastNonZero =
            stbiw__jpg_dct_DU(DUs[div3][mod3], CDU, duStride, fdtbl);
                               ...
                               fixed_point zero)
{
    ...
    // Write Headers
    {
        ...
        stbi__stdio_write(s->context, (void *)(std_dc_luminance_nrcodes + 1),
...
#define BLOCK_SIZE 8 // may not be larger than 42
            ...
            for (int i = 0; i < numIter; i += BLOCK_SIZE)
            {
                ...
                int subIterCount =
                    i + BLOCK_SIZE >= numIter ? numIter - i : BLOCK_SIZE;

                int32_t magicNumber = ((1 << 24) + numIterW - 1) / numIterW;

                // Converting color space from RGB to YUV
                stbiw__convert_colors_16(
                    i, subIterCount, numIterW, numIterH, magicNumber, height,
                    width, comp, width * comp, dataR, dataG, dataB, Ys, Us, Vs);
                ...
                stbiw__jpg_complete_du_16(subIterCount, Ys, subUs, subVs,
                                          fdtbl_Y, fdtbl_UV, lastNonZeroIdxes,
                                          DUs);
                ...
                for (int j = 0; j < subIterCount; j++)
                {
                    ...
                    DCY = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][1], DCY, YDC_HT, YAC_HT,
                        &filePos, lastNonZeroIdxes[j][1]);
                    DCY = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][2], DCY, YDC_HT, YAC_HT,
                        &filePos, lastNonZeroIdxes[j][2]);
                    DCY = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][3], DCY, YDC_HT, YAC_HT,
                        &filePos, lastNonZeroIdxes[j][3]);
                    DCU = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][4], DCU, UVDC_HT, UVAC_HT,
                        &filePos, lastNonZeroIdxes[j][4]);
                    DCV = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][5], DCV, UVDC_HT, UVAC_HT,
                        &filePos, lastNonZeroIdxes[j][5]);
                }
...
#define BLOCK_SIZE 8 // may not be larger than 42
            ...
            for (int i = 0; i < numIter; i += BLOCK_SIZE)
            {
                ...
                stbiw__jpg_complete_du_8(subIterCount, Ys, Us, Vs, fdtbl_Y,
                                         fdtbl_UV, lastNonZeroIdxes, DUs);
                ...
                for (int j = 0; j < subIterCount; j++)
                {
                    DCY = stbiw__jpg_encode_DC_AC(
                        ...
                        &filePos, lastNonZeroIdxes[j][0]);
                    DCU = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][1], DCU, UVDC_HT, UVAC_HT,
                        &filePos, lastNonZeroIdxes[j][1]);
                    DCV = stbiw__jpg_encode_DC_AC(
                        s, &bitBuf, &bitCnt, DUs[j][2], DCV, UVDC_HT, UVAC_HT,
                        &filePos, lastNonZeroIdxes[j][2]);
                }
        ...
        output_arr_length = stbiw__jpg_writeBits_direct(
            s, &bitBuf, &bitCnt, fillBits, output_arr_length);
    }
